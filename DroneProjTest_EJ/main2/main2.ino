...
class PID {
    public:
        PID(float startingVal, float pw, float iw, float dw) {
            p = startingVal;
            Integrator iVal = new Integrator(p);
            lastVal = p;
            d = 0;
        ...
        void addValue(float newVal, float timeStep) {
            pVal = newVal;
            iVal.addValue(newVal, elapsedTime);
            dVal = getDerivative(lastVal, newVal, timeStep);
            lastVal = newVal;
        }
    ...
    private:
        float pWeight, iWeight, dWeight, pVal, iVal, dVal, lastVal;
}
...
float rotateArray(float[3] tuple, )
