...
bool isStarted() {
    ...
    if (status == STOPPED && pulse_length[mode_mapping[YAW]] <= 1050 && pulse_length[mode_mapping[THROTTLE]] <= 1050) {
        status = STARTING;
    }

    // When left stick is moved back in the center position
    if (status == STARTING && abs(pulse_length[mode_mapping[YAW]] - 1500) <= 50 && abs(pulse_length[mode_mapping[THROTTLE]] - 1500) <= 50) {
        status = STARTED;
    ...
    if (status == STARTED && pulse_length[mode_mapping[YAW]] >= 1950 && pulse_length[mode_mapping[THROTTLE]] <= 1050) {
        status = STOPPED;
